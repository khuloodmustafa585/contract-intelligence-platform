def build_summary_prompt(contract_text: str) -> str:
    ...
    # Returns a tuple so callers can pass system + user as separate messages.
    # For backward-compat with get_prompt_by_task (which expects a single string),
    # the combined form is returned as the default.
    return f"{system}\n\n---\n\n{user}"
